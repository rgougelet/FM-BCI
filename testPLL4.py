...
from matplotlib.pyplot import figure, title, xlabel, ylabel, plot, grid, show, axis, legend
from numpy import pi

fig_count = 1

# frequency modulation parameters
alphaCenter = 15   # Hz the carrier frequency
alphaModFreq = 1  # Hz the modulating frequency
alphaFreqDev = 0.5    # Hz of the frequency deviation, BW is 2x this
...
#order = 8
#b, a = sp.signal.bessel(order, alphaCenter/nyq, btype='low')
#w,h = signal.freqz(b,a)

order = sampleRate
stopfreq = alphaCenter/nyq
...
filteredPhaseDiff = np.zeros(dataLengthSamples+order)
unfilteredPhaseDiff = np.zeros(dataLengthSamples+order)
print delaysamples
...
for gainIndex in range(1):
    ...
    for sampleInd in range(0,dataLengthSamples-1):
        unfilteredPhaseDiff[sampleInd+order] = channelVoltage[sampleInd]*SCO[sampleInd] # phase difference of reference vs. signal
       
        #filter the multiplied signals
        fac = 1
        gain = 0.001
        unfilteredPhase_chunk = unfilteredPhaseDiff[sampleInd+order:sampleInd+order-chunkSize:-1] # chunk of unfiltered phase difference
        ...
        filteredPhaseDiff[sampleInd+order] = dotted_unfilteredPhase_chunk
...
fig_count += 1
plot(unfilteredPhaseDiff[order:], 'g', label='Unfiltered Phase Diff') # unfiltered phase difference
plot(filteredPhaseDiff[order:], 'r', label='Filtered Phase Diff') # filtered phase difference
...
plot(filteredPhaseDiff[order:]/(w0*Ts*np.pi)+alphaCenter, 'b', label='Tracked Freq')
plot(actual_freq, 'g', label='Actual Freq')
legend(loc='bottom right')
