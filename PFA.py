...
def processPAF(voltageSamples, sampleRate):
    ...
    for channelIndex in range(0, numOfChannel):
        ...
        channelVoltage = voltageSamples[
            channelIndex, :] - np.mean(voltageSamples[channelIndex, :])
        #windowed = channelVoltage * signal.blackmanharris(dataLengthSamples)
        windowed = channelVoltage * \
            signal.gaussian(dataLengthSamples, std=8, sym=False)
        ...
        print('Channel ' + str(channelIndex + 1) + ':     ' +
              str(true_maxFreq) + '  ' + str(maxFreq))


        # target.write('Channel ' + str(channelIndex + 1) + ':     ' +
        #       str(true_maxFreq) + '  ' + str(maxFreq))
        # target.write("/n")
