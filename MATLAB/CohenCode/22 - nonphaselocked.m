...
itpc_nonphase = zeros(num_frex,EEG.pnts);
...
for fi=1:length(frex)
    ...
    for methodi=1:3
        ...
        if methodi==2
            itpc_nonphase(fi,:) = abs(mean(exp(1i*angle(as)),2));
        end
        if methodi<3
...
analysis_labels = {'Total';'Non-phase-locked';'TF of Average Time Series across Trials (ERP)';'ITPC'};
...
meanitpc = mean(squeeze(tf(4,:,:)),1);
subplot(2,3,6)
contourf(EEG.times,frex,itpc_nonphase,40,'linecolor','none')
set(gca,'clim',clims(methodi,:),'xlim',[-400 1000],'xtick',-200:200:800)
xlabel('Time (ms)')
ylabel('Frequency (Hz)')
